...
def story_run(story, current_path, previous_path):
    ...
    if story[current_path]["options"] != "K": #if it is not kill, continue story
        ...
        while True:
            try:
                ...
                if user_choice in (1, 2, 3):
                    ...
                    type_print("Please choose between 1-3:")
            except ValueError:
...
show_death_screen() 
        while True:
            user_choice_YN = input("\nPress Y or N to try again? (Y/N capital letters)\n")
    ...
    current_path = story[current_path]["options"][f"{user_choice}"]["path"] #well, its the next path but yeah
    return current_path, previous_path
